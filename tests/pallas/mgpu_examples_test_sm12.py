...
def matmul6(a, b, config: TuningConfig):
  ...
  if k_iters % max_concurrent_steps != 0:
    raise ValueError(f"{k_iters=} must be divisible by {max_concurrent_steps=}")

  grid_size = min(num_sms * config.gs_num_sms_factor, m_iters * n_iters)

  itemsize = jnp.dtype(dtype).itemsize  # in bytes
  swizzle = plgpu.find_swizzle(tile_k * itemsize * 8)
  swizzle_elems = swizzle // itemsize
  a_transforms = (
  ...
  out_swizzle = plgpu.find_swizzle(config.epilogue_tile_n * itemsize * 8)
  out_swizzle_elems = out_swizzle // itemsize
  out_transforms = (
  ...
  @plgpu.kernel(
      ...
      scratch_types=dict(
          ...
          out_smem=plgpu.SMEM((2, tile_m, config.epilogue_tile_n), dtype, transforms=out_transforms),
          load_barriers=plgpu.Barrier(num_arrivals=2, num_barriers=max_concurrent_steps),  # gmem to smem copy barriers
          consumed_barriers=plgpu.Barrier(num_arrivals=1, num_barriers=max_concurrent_steps),
      ),
      num_threads=2, # Two warpgroups (256 threads) per block
